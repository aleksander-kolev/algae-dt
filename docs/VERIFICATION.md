...
Every deliverable maps to a rubric pillar and to concrete, re-runnable evidence. **103 tests pass**
...
| `lib/sync.py` | pose/sensor error + tolerances + classify + commanded-shadow unicycle | `test_sync.py` (14) |
| `lib/metrics.py` | command→motion latency, data age, inf-safe CSV row/header | `test_metrics.py` (8) |
| `lib/geometry.py` | world↔pixel, yaw↔quaternion, angle wrap | `test_geometry.py` (5) |
| `lib/pgm.py` | P5/P2 parser incl. the real 86×110 course map | `test_pgm.py` (9) |
| `lib/hud.py` | battery colour thresholds, scan projection, status text | `test_hud.py` (4) |
...
- Mediator integration: `test_mediator.py` (4).
...
- **Proof:** `test_sync_supervisor.py` (4) — in-tolerance vs out-of-tolerance flip + alert, latency
...
- **25 cm dual-LiDAR stop on BOTH worlds, fail-safe** — `test_safety.py` (gate logic) +
  `test_mediator.py` (live zeroing of forward + `/dt/safety`) +
  `test_fake_robot.py::test_front_obstacle_triggers_safety_stop_through_mediator` (hardware-free).
- **Navigate-and-spray with honest accounting** — `test_mission_runner.py` (3): SUCCEEDED→spray+treated,
...
proven by `mission_smoke.py`) but rarely *arrives* headless. Run the full navigate-and-spray demo on
