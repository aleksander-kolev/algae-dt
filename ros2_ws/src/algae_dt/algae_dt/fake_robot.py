...
it actually "moves" it. This makes pillar ① (real->digital and digital->real arrows) and the
topic-collision rule demonstrable WITHOUT hardware, and lets `both` mode be validated at home (the
real side = fake_robot on bare topics, the sim on /sim/*). Pure kinematics live in lib.sync; this is
ROS wiring. NOT used in sim_only (the Gazebo robot is the bare robot there).
"""
