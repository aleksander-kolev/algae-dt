"""World <-> map-pixel transforms and small 2D math helpers. No ROS dependencies.

Map parameters come from twin.yaml.
"""
...
def world_to_pixel(x: float, y: float, m: MapInfo) -> tuple[int, int]:
    ...
    round-trips exactly: world_to_pixel(pixel_to_world(c,r)) == (c,r). Doing
