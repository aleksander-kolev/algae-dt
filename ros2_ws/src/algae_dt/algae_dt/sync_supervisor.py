"""sync_supervisor — real-time synchronisation and tolerance monitoring.

Measures the real-vs-sim discrepancy and reports it, not just a yes/no:
...
and /dt/alerts (String) when something is out of the twin.yaml tolerances, and appends a flushed
CSV row each tick. The maths live in lib.sync + lib.metrics; this node is the ROS wiring and I/O.
"""
...
class SyncSupervisor(Node):
    ...
    def _stream_ok(self) -> bool:
        """True iff both pose streams have reported within pose_timeout_s."""
        now = self._now()
    ...
    def _on_sync_timer(self) -> None:
        if not self._stream_ok():
            ...
            # is a real desync -> fail loud (sync_ok False + alert), never silently skip.
        ...
        if self._csv:
            ...
            self._csv.flush()
        self._pending_stop_skew = None     # reset every tick
