"""bringup.launch.py — brings up the stock turtlebot3 stack plus our algae_dt layer.
...
Topic model: the active robot is on bare topics (/scan,/odom,/cmd_vel,/battery_state)
...
cmd_vel chokepoint: stock burger.yaml already sets
...
gate before reaching /cmd_vel (plus the /sim/cmd_vel mirror in both). sim_only flips
set_initial_pose:true for headless AMCL auto-seed (use_sim_time is true only in sim_only).
"""
import os
...
def launch_setup(context, *args, **kwargs):
    ...
    # Real side at home (no hardware): fake_robot publishes the bare /scan /odom /cmd_vel(sub)
    # /battery_state so real_only/both can be exercised without a robot.
    ...
    actions += [
        ...
        # Never set name= on mission_runner (it triggers a process-wide remap that breaks topics).
